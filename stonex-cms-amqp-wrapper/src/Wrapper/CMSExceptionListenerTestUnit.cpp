...
void CMSExceptionListenerTestUnit::onException(const cms::CMSException& ex)
{
	if (mListener)
		mListener->onException(CMSExceptionTestUnit(mId,ex));
}

void CMSExceptionListenerTestUnit::attachListener(cms::ExceptionListener* listener)
{
	mListener = listener;
}

std::string CMSExceptionListenerTestUnit::id() const
{
	return mId;
}
